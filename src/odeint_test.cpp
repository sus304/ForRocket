...
class Rocket
{
    ...
    void dynamics(const state& x, state& dx, const double t)
    {
        double pos = x[0];
        double vel = x[1];
        double F = 0.0;
...
struct Observer
{
    ...
    void push(const state& x, state& dx, double t)
    {
...
void solver()
{
    ...
    // odeint::integrate_const(stepper, rocket:;dynamics, state0, 0.0, 25.0, 0.1, std::ref(observer))
    odeint::integrate_const(stepper, rocket.dynamics, state0, 0.0, 25.0, 0.1, observer.push);
