\documentclass[a4paper]{jsarticle}

\usepackage{bm}
\usepackage{here}
\usepackage{amsmath}

\begin{document}

\title{ForRocket \\ Technical Document}
\author{Susumu Tanaka}
\maketitle

\section{Introduction}
6DoFロケットフライトシミュレータであるForRocketに実装されている計算モデルをまとめる。



\section{Model}
\subsection{座標系}

\begin{table}[htb]
  \centering
  \begin{tabular}{|l|l|l|l|}
  \hline
  座標系                                                                                      & 記号                                                                & 原点   &                                                                                                        \\ \hline
  \begin{tabular}[c]{@{}l@{}}地球中心慣性座標系\\ ECI\\ (Earth Center, Inertia)\end{tabular}        & \begin{tabular}[c]{@{}l@{}}I\\ ($X^I$, $Y^I$, $Z^I$)\end{tabular} & 地球中心 & \begin{tabular}[c]{@{}l@{}}$X^I$:$t=0$で緯度経度0 deg方向\\ $Y^I$:右手直交座標となる方向\\ $Z^I$:地球自転軸で北極方向\end{tabular} \\ \hline
  \begin{tabular}[c]{@{}l@{}}地球中心地球固定座標系\\ ECEF\\ (Earth Center, Earth Fixed)\end{tabular} & \begin{tabular}[c]{@{}l@{}}E\\ ($X^E$, $Y^E$, $Z^E$)\end{tabular} & 地球中心 & \begin{tabular}[c]{@{}l@{}}$X^E$:緯度経度0 deg方向\\ $Y^E$:右手直交座標となる方向\\ $Z^E$:地球自転軸で北極方向\end{tabular}       \\ \hline
  \begin{tabular}[c]{@{}l@{}}局所水平面座標系\\ NED\\ (North, East, Down)\end{tabular}             & \begin{tabular}[c]{@{}l@{}}L\\ ($X^L$, $Y^L$, $Z^L$)\end{tabular} & 機体重心 & \begin{tabular}[c]{@{}l@{}}$X^L$:局所水平面内で北方向\\ $Y^L$:局所水平面内で東方向\\ $Z^L$:水平面垂直に地球中心方向\end{tabular}       \\ \hline
  \begin{tabular}[c]{@{}l@{}}機体座標系\\ BODY\end{tabular}                                     & \begin{tabular}[c]{@{}l@{}}B\\ ($X^B$, $Y^B$, $Z^B$)\end{tabular} & 機体重心 & \begin{tabular}[c]{@{}l@{}}$X^B$:機軸方向(ロール軸)\\ $Y^B$:頭上げ方向(ピッチ軸)\\ $Z^B$:右首振り方向(ヨー軸)\end{tabular}       \\ \hline
  \begin{tabular}[c]{@{}l@{}}測地座標系\\ LLH\\ (Latitude, Longitude, Height)\end{tabular}      & \begin{tabular}[c]{@{}l@{}}G\\ ($X^G$, $Y^G$, $Z^G$)\end{tabular} & -    & \begin{tabular}[c]{@{}l@{}}$X^G$:地理緯度 {[}deg{]}\\ $Y^G$:経度 {[}deg{]}\\ $Z^G$:海抜高度 {[}m{]}\end{tabular} \\ \hline
  \end{tabular}
\end{table}

\subsection{座標変換}
異なる座標系で表現されているベクトルの座標系変換は座標変換行列を用いて表現する。
座標変換行列は方向余弦行列 (Direct Cosine Matrix, DCM) で定義され、
J座標系をK座標系に変換するDCMを $C^{KJ}$ と表す。

例えば、I系の速度ベクトル $v^I$ をE系に変換するには
\begin{equation}
v^E = C^{EI} \cdot v^I
\end{equation}
となる。


\subsection{並進運動方程式}
並進運動では慣性座標系で定義した運動方程式を解く。
\begin{eqnarray}
\frac{d\bm{r}^I}{dt} &=& \bm{v}^I \\
\frac{d\bm{v}^I}{dt} &=& \frac{\bm{F}^I}{m} \\
&=& \frac{C^{IB} \cdot \bm{F}^B}{m}
\end{eqnarray}


\subsection{外力}
運動方程式中に表れる機体座標系の外力 $F^B$ の詳細について記す。
\begin{equation}
\bm{F}^B = \bm{F}^B_T + \bm{F}^B_A + \bm{F}^B_g
\end{equation}

\subsubsection{ロケットエンジン/モータ推力}
ロケットエンジンによる推力 $T$ を機体座標系で定義すれば
\begin{equation}
\bm{F}^B_T = [T, 0, 0]^T
\end{equation}
ForRocketにおける推力入力は真空中推力[N]であるため、大気圧環境下では圧力推力補正をかける。
\begin{equation}
T = T_{vac} - P_a A_e
\end{equation}
さらに、機体軸に対する推力軸のミスアライメントを$\varepsilon_y$、$\varepsilon_z$で定義すると
\begin{equation}
\bm{T}^B = T \cdot [\cos{\varepsilon_y} \cos{\varepsilon_z}, \sin{\varepsilon_y}, -\sin{\varepsilon_y}]^T
\end{equation}
したがって、機体座標系の推力 $\bm{F}^B_T$ は以下となる。
\begin{equation}
\bm{F}^B_T = (T_{vac} - P_aA_e) \cdot [\cos{\varepsilon_y} \cos{\varepsilon_z}, \sin{\varepsilon_y}, -\sin{\varepsilon_y}]^T
\end{equation}

\subsubsection{空気力}
空気力は機体座標系で定義し、軸力 $F_A$ と法線力 $F_N$ で扱う。航空機でよく用いられる抗力 $F_D$ と揚力 $F_L$ とは軸が異なるため注意。

風の影響を含めるため対気速度 $\bm{v}_{air}$ から迎角 $\alpha$ 、横滑り角 $\beta$ と動圧 $Q$ を算出する。
\begin{eqnarray}
\bm{v}_{air}^B &=& C^{BL} \cdot (\bm{v}^L - \bm{v}_{wind}^L) \\
\alpha &=& \arcsin{\frac{\bm{v}_{air,z}}{\begin{vmatrix}\bm{v}_{air}\end{vmatrix}}} \\
\beta &=& \arcsin{\frac{-\bm{v}_{air,y}}{\begin{vmatrix}\bm{v}_{air}\end{vmatrix}}} \\
Q &=& \frac{1}{2} \rho \begin{vmatrix}\bm{v}_{air}\end{vmatrix}^2
\end{eqnarray}
yおよびz軸の法線力と軸力で機体座標系の空力を表す。
\begin{eqnarray}
\bm{F}_A^B &=& [-F_A, F_{N,y}, -F_{N,z}]^T  \\
F_A &=& Q \cdot C_{A} \cdot S \\
F_{N,z} &=& Q \cdot C_{N\alpha} \cdot S \cdot \alpha \\
F_{N,y} &=& Q \cdot C_{N\alpha} \cdot S \cdot \beta
\end{eqnarray}
なお、軸力係数 $C_A$ および法線力傾斜 $C_{N\alpha}$ はMach数の関数として入力される。
本来は迎角の関数でもあるが、ロケットは大きな迎角を持って飛行することが少なく、影響が限定的であるため無視している。一応、ForRocketの機能として、入力処理はないものの、Mach数と迎角の2変数関数として扱う機能は実装されている。


\subsubsection{重力}
重力はL系で定義し、地球を球とした簡易計算を行う。
\begin{eqnarray}
g &=& \frac{GM}{h^2} \\
\bm{g}^L &=& [0, 0, g]^T
\end{eqnarray}

外力としての重力は
\begin{equation}
\bm{F}_g^B = m \cdot C^{BL} \cdot \bm{g}^L
\end{equation}
で計算する。


\subsection{回転運動方程式}
回転運動では機体座標系で定義した運動方程式を解く。機体座標系は剛体に固定された座標系であるため、見かけの力としてジャイロモーメントを加える。
\begin{equation}
\bm{M}^B = \frac{d\bm{L}}{dt} + \bm{\omega^B} \times \bm{L}
\end{equation}
$L = I \omega$を用いれば
\begin{equation}
\bm{M}^B = 
\dot{\bm{I}} \bm{\omega^B}
+ \bm{I} \dot{\bm{\omega}}^B
+ \bm{\omega^B} \times \bm{I} \bm{\omega^B}
\end{equation}
となり、整理すれば
\begin{equation}
\bm{I} \dot{\bm{\omega}}^B = 
\bm{M}^B
- \dot{\bm{I}} \bm{\omega^B}
- \bm{\omega^B} \times \bm{I} \bm{\omega^B}
\end{equation}
が解くべき回転の運動方程式となる。
ただし、慣性テンソルが一定であると仮定すれば第2項を省略でき
\begin{equation}
\bm{I} \dot{\bm{\omega}}^B = 
\bm{M}^B
- \bm{\omega^B} \times \bm{I} \bm{\omega^B}
\end{equation}
として扱うことができる。
慣性モーメントの変化が小さいロケットであれば、この仮定でも大きな影響はないと思われる。

一般的に回転の運動方程式で目にするのはこれを展開した式となる。
\begin{equation}
\bm{\omega}^B = [p, q, r]^T
\end{equation}
\begin{eqnarray}
I_{xx} \dot{p} &= (I_{yy}-I_{zz})qr+M_p \\
I_{yy} \dot{q} &= (I_{zz}-I_{xx})rp+M_q \\
I_{zz} \dot{r} &= (I_{xx}-I_{yy})pq+M_r
\end{eqnarray}

スピン安定解析を目的として、機体主軸と機軸が一致しない（製造誤差等で慣性主軸が傾いている）場合の動的アンバランスを表現するため、慣性テンソルに慣性乗積 $I_{xy}, I_{xz}, I_{yz}$ も含めて定義する。
\begin{equation}
\bm{I} =
\begin{bmatrix}
 I_{xx} & -I_{xy} & -I_{xz} \\
-I_{xy} &  I_{yy} & -I_{yz} \\
-I_{xz} & -I_{yz} &  I_{zz}
\end{bmatrix}
\end{equation}
慣性乗積はいずれも省略時 0 とし、その場合は主軸のみの対角テンソルとなり従来挙動と一致する。実装上は $\bm{\omega}^B \times (\bm{I} \bm{\omega}^B)$ のジャイロ項計算に直接乗積成分が含まれるため、上記運動方程式のクロスカップリング項に追加の補正を加える必要はない。

\subsection{モーメント}
運動方程式中に表れる機体座標系のモーメント $M^B$ の詳細について記す。

\begin{equation}
\bm{M}^B = \bm{M}^B_T + \bm{M}^B_A + \bm{M}^B_{AD} + \bm{M}^B_{JD} + \bm{M}^B_{GJ}
\end{equation}

\subsubsection{推力モーメント}
推力軸のミスアライメント角、および推力作用点・重心の機軸からの横方向オフセットによって発生する。スピン安定解析を目的として、重心 $\bm{r}_{CG} = [X_{CG}, Y_{CG}, Z_{CG}]^T$ および推力入力点 $\bm{r}_T = [X_T, Y_T, Z_T]^T$ をそれぞれ機体座標系の 3 次元ベクトルとして与え、その差分をモーメントアームとして計算する。
\begin{eqnarray}
\bm{r} &=& \bm{r}_{CG} - \bm{r}_T = [X_{CG} - X_T,\ Y_{CG} - Y_T,\ Z_{CG} - Z_T]^T \\
\bm{M}^B_T &=& \bm{F}^B_T \times \bm{r}
\end{eqnarray}
横方向オフセット $Y_{CG}, Z_{CG}, Y_T, Z_T$ はいずれも省略時 0 で、その場合は従来の機軸上のみの定式化と一致する。推力ベクトル $\bm{F}^B_T$ にはエンジンミスアライメント角（既存）が反映される。
なお入力する横方向重心オフセットは乾燥構造（インート）部のものであり、v4.4.2 より実効的な横方向重心は推進薬（機軸上にあると仮定）との質量重み付けで算出される。乾燥質量を $m_{inert}$、推進薬質量を $m_{prop}$ として
\begin{equation}
Y_{CG} = Y_{CG,inert} \cdot \frac{m_{inert}}{m_{inert} + m_{prop}}
\end{equation}
（$Z_{CG}$ も同様）であり、推進薬の消費に伴いインート値へ漸近する。

\subsubsection{空気力モーメント}
外力として計算されている機体座標系での空気力ベクトルと、重心と圧力中心位置 $X_{CP}$ の距離をモーメントアームとして計算する。
\begin{eqnarray}
\bm{r}_A &=& [X_{CG} - X_{CP}, 0, 0]^T \\
\bm{M}^B_A &=& \bm{F}^B_A \times \bm{r}_A
\end{eqnarray}
ロール軸の空気力モーメントについては尾翼のカント角 $\delta$ によるモーメントのみを考慮する。
カント角によるロールモーメント係数 $C_{l\delta}$ を用いて
\begin{equation}
M^B_{A,x} = Q \cdot C_{l\delta} \cdot S \cdot d \cdot \delta
\end{equation}
で計算する。
ここで $d$ は機体直径、$\delta$ は尾翼1枚あたりのカント角である。
基準長は v4.2.2 より機体全長 $L$ から機体直径 $d$ に変更された。
また $C_{l\delta}$ は尾翼全体（クロスフィン）合計の値を入力する（v4.2.2 より旧版の「1枚あたり係数 $\times 4$」を廃止）。入力する $C_{l\delta}$ の次元に留意すること。

\subsubsection{空気力減衰モーメント}
空気力減衰モーメントは各軸の減衰モーメント係数を用いて計算する。
\begin{eqnarray}
C_{ad} &=& [C_{lp}, C_{mq}, C_{nr}]^T \\
\bm{M}^B_{AD} &=& Q \cdot C_{ad} \cdot S \cdot \frac{d^2}{2 \bm{v^B}} \cdot \omega^B
\end{eqnarray}
ここで基準長は機体直径 $d$ である（v4.2.2 より機体全長 $L$ から変更）。減衰モーメント係数 $C_{lp}, C_{mq}, C_{nr}$ は $d$ で無次元化した値を入力する。軸対称のロケットであれば $C_{mq}$ と $C_{nr}$ は同等である。

\subsubsection{ジェット減衰モーメント}
ロケットエンジンの排気がノズル出口で角運動量を持ち去ることによる減衰モーメントである（v4.4.2 で実装）。
質量流量を $\dot{m}$、機体角速度を $\bm{\omega}^B = [p, q, r]^T$ として各軸を次式で計算する。
\begin{eqnarray}
M^B_{JD,x} &=& -\dot{m} \cdot \frac{A_e}{2\pi} \cdot p \\
M^B_{JD,y} &=& -\dot{m} \cdot l^2 \cdot q \\
M^B_{JD,z} &=& -\dot{m} \cdot l^2 \cdot r
\end{eqnarray}
ここで $A_e$ はノズル出口面積、$l$ は重心からノズル出口までの軸距離（機体後端 $\approx$ ノズル出口と仮定し $l \approx L_{CG}$）である。
ロール軸の腕の長さ二乗 $A_e/(2\pi)$ は、排気が機軸まわりに一様分布する中央単ノズルを仮定した慣動半径二乗 $r_e^2/2$ に相当する。

\subsubsection{ガスジェットモーメント}
スピン安定化を目的としたコールドガスジェットによるロールモーメントを表す。
ランチクリア（レール離脱）を検出した時刻 $t_{LC}$ を起点とし、設定された作動時間 $\Delta t_{GJ}$ の間のみモーメントを印加する。

\begin{equation}
\bm{M}^B_{GJ} =
\begin{cases}
[M_{GJ}, 0, 0]^T & (t_{LC} \leq t \leq t_{LC} + \Delta t_{GJ}) \\
[0, 0, 0]^T & (\text{otherwise})
\end{cases}
\end{equation}

ここで $M_{GJ}$ はロール軸（機体 $X^B$ 軸）まわりの入力ローリングモーメント [N·m] であり、正値でスピンアップ方向とする。
$M_{GJ}$ および $\Delta t_{GJ}$ はユーザー入力パラメータとして与える。
レール離脱の検出は、ランチャ走行距離 $d = \Delta h / \sin\theta$ が有効レール長 $L_{rail}$ に達した時刻として行われる（時刻ベースではなく位置ベースの検出）。


\subsection{オイラー角}
姿勢表示にはL系とB系間のオイラー角を用いる。

\begin{table}[htb]
  \centering
  \begin{tabular}{|l|l|l|}
  \hline
  姿勢角                                                              & 定義                                   & 対応するB系回転軸                                                 \\ \hline
  \begin{tabular}[c]{@{}l@{}}方位角\\ Azimuth $\psi$\end{tabular}     & $X^L-Y^L$ 面への $X^B$ 軸の射影と $X^L$ がなす角 & \begin{tabular}[c]{@{}l@{}}$Z^B$ 軸回転\\ ヨー回転\end{tabular}  \\ \hline
  \begin{tabular}[c]{@{}l@{}}上下角\\ Elevation $\theta$\end{tabular} & $X^B$ 軸と $X^L-Y^L$ 面がなす角             & \begin{tabular}[c]{@{}l@{}}$Y^B$ 軸回転\\ ピッチ回転\end{tabular} \\ \hline
  \begin{tabular}[c]{@{}l@{}}ロール角\\ Roll $\phi$\end{tabular}       & $X^B$ 軸まわりの回転角                       & \begin{tabular}[c]{@{}l@{}}$X^B$ 軸回転\\ ロール回転\end{tabular} \\ \hline
  \end{tabular}
\end{table}

\subsection{クォータニオン}
オイラー角によるL系からB系への座標変換行列を表すと
\begin{equation}
C^{BL} = 
\begin{bmatrix}
 \cos\psi \cos\theta & sin\psi \cos\theta & -sin\theta \\
 -sin\psi \cos\phi + \cos\psi sin\theta sin\phi & \cos\psi \cos\phi + sin\psi sin\theta sin\phi & \cos\theta sin\phi \\
 sin\psi sin\phi + \cos\psi sin\theta \cos\phi & -\cos\psi sin\phi + sin\psi sin\theta \cos\phi & \cos\theta \cos\phi
\end{bmatrix}
\end{equation}
となるが、$\theta=\pm{\frac{\pi}{2}}$ の時に
\begin{equation}
C^{BL} = 
\begin{bmatrix}
 0 & 0 & -1 \\
 -sin\psi \cos\phi + \cos\psi sin\phi & \cos\psi \cos\phi + sin\psi sin\phi & 0 \\
 sin\psi sin\phi + \cos\psi \cos\phi & -\cos\psi sin\phi + sin\psi \cos\phi & 0
\end{bmatrix}
\end{equation}
となるため、1軸 (方位角) の回転しか表せない。
このようなオイラー角の特異点を回避するため、計算ではクォータニオンを用いる。

クォータニオンを用いたL系からB系への座標変換行列は
\begin{equation}
C^{BL} = 
\begin{bmatrix}
 q_0^2 - q_1^2 - q_2^2 + q_3^2 & 2 (q_0  q_1 + q_2  q_3) & 2 (q_0  q_2 - q_1  q_3) \\
 2 (q_0  q_1 - q_2  q_3) & q_1^2 - q_0^2 - q_2^2 + q_3^2 & 2 (q_1  q_2 + q_0  q_3) \\
 2 (q_0  q_2 + q_1  q_3) & 2 (q_1  q_2 - q_0  q_3) & q_2^2 - q_0^2 - q_1^2 + q_3^2
\end{bmatrix}
\end{equation}
となり、クォータニオンの時間微分は
\begin{equation}
\bm{\omega}^B = [p, q, r]^T
\end{equation}
\begin{equation}
\frac{d\bm{q}}{dt} = \frac{1}{2}
\begin{bmatrix}
 0 & r & -q & p \\
 -r & 0 & p & q \\
 q & -p & 0 & r \\
 -p & -q & -r & 0
\end{bmatrix}
\bm{q}
\end{equation}
となる。姿勢の積分にはこちらを用いる。


\subsection{離散化}
並進、回転ともにDormand-Princeの方法で離散化を行い、微分方程式を解く。

Runge-Kutta-Dormand-Prince (RKDP) は陽的ルンゲクッタ法の1種である。
7段5次の精度だが、FSAL(First Same As Last)という性質を持つため実質的に6段5次の精度を有する。
また、埋込み型公式のため、刻み幅自動調整が可能である。

RKDPでの1ステップの計算が
\begin{eqnarray}
k_1 &=& hf(t_k, y_k) \\
k_2 &=& hf(t_k + \frac{1}{5}h, y_k + \frac{1}{5}k_1) \\
k_3 &=& hf(t_k + \frac{3}{10}h, y_k + \frac{3}{40}k_1 + \frac{9}{40}k_2) \\
k_4 &=& hf(t_k + \frac{4}{5}h, y_k + \frac{44}{45}k_1 - \frac{56}{15}k_2 + \frac{32}{9}k_3) \\
k_5 &=& hf(t_k + \frac{8}{9}h, y_k + \frac{19372}{6561}k_1 - \frac{25360}{2187}k_2 + \frac{64448}{6561}k_3 - \frac{212}{729}k_4) \\
k_6 &=& hf(t_k + h, y_k + \frac{9017}{3168}k_1 - \frac{355}{33}k_2 - \frac{46732}{5247}k_3 + \frac{49}{176}k_4 - \frac{5103}{18656}k_5) \\
k_7 &=& hf(t_k + h, y_k + \frac{35}{384}k_1 + \frac{500}{1113}k_3 + \frac{125}{192}k_4 - \frac{2187}{6784}k_5 + \frac{11}{84}k_6) 
\end{eqnarray}
となり、次ステップの値が
\begin{equation}
y_{k+1} = y_k + \frac{35}{384}k_1 + \frac{500}{1113}k_3 + \frac{125}{192}k_4 - \frac{2187}{6784}k_5 + \frac{11}{84}k_6
\end{equation}
となる。
さらに誤差評価用の計算を行う。
\begin{equation}
t_{k+1} = y_k + \frac{5179}{57600}k_1 + \frac{7571}{16695}k_3 + \frac{393}{640}k_4 - \frac{92097}{339200}k_5 + \frac{187}{2100}k_6 + \frac{1}{40}k_7
\end{equation}
$y_{k+1}$ の誤差として差分をとれば
\begin{equation}
\begin{vmatrix}t_{k+1} - y_{k+1}\end{vmatrix} = 
\begin{vmatrix}
\frac{71}{57600}k_1
- \frac{71}{16695}k_3
+ \frac{71}{1920}k_4
- \frac{17253}{339200}k_5
+ \frac{22}{525}k_6
- \frac{1}{40}k_7
\end{vmatrix}
\end{equation}
この誤差から求める刻み修正係数 $s$ に安全率0.9をかけて新たな刻み幅 $h_o$ を決定する。
\begin{equation}
s = 0.9 \biggl(\frac{\varepsilon h}{\begin{vmatrix}t_{k+1} - y_{k+1}\end{vmatrix}}\biggr)^\frac{1}{5}
\end{equation}
\begin{equation}
h_o = s \cdot h
\end{equation}
なお、$\varepsilon$ は許容誤差限界である。

\subsection{スピン安定・ロールピッチレゾナンス評価}
スピン安定方式を採用する場合、スピン（ロール）レート $p$ がピッチ／ヨーの空力固有振動数 $\omega_n$ に接近すると、機体固定の構成非対称（質量・空力アンバランス）がピッチ／ヨー運動を共振励起し、迎角が急増する。これをロールピッチ（スピン-ピッチ／ロール-ヨー）レゾナンスと呼ぶ。本節の各量はフライトログに出力される評価指標であり、各時刻の状態量から後処理的に算出される（運動方程式本体には影響しない）。

横慣性 $I_T$、静的マージン距離 $\ell$、ピッチ／ヨー空力復元剛性 $k_\alpha$ を次で定義する。$Q$ は動圧、$S$ は基準面積、$C_{N\alpha}$ は法線力傾斜、$X_{CG}, X_{CP}$ は機体後端からの重心・圧力中心位置である。
\begin{eqnarray}
I_T &=& \frac{I_{yy} + I_{zz}}{2} \\
\ell &=& X_{CG} - X_{CP} \\
k_\alpha &=& Q\,S\,C_{N\alpha}\,\ell
\end{eqnarray}
$\ell>0$（CP が CG より後方）で静安定である。ピッチ／ヨー空力固有角振動数 $\omega_n$、固有振動数 $f_n$、スピン周波数 $f_s$、および共振比 $\Lambda$ は
\begin{eqnarray}
\omega_n &=& \sqrt{\frac{k_\alpha}{I_T}} \\
f_n &=& \frac{\omega_n}{2\pi}, \qquad f_s = \frac{|p|}{2\pi} \\
\Lambda &=& \frac{|p|}{\omega_n}
\end{eqnarray}
で与えられ、$\Lambda \approx 1$ で共振条件となる。$\Lambda$ が設計判定の主指標である。全迎角は迎角 $\alpha$ と横滑り角 $\beta$ から
\begin{equation}
\alpha_t = \sqrt{\alpha^2 + \beta^2}
\end{equation}
とする。

ピッチ／ヨー振動の減衰は、CP–CG オフセットによる揚力減衰 $c_L$（フィン安定機の支配項）と $C_{mq}$ 減衰モーメント $c_{Cmq}$ の和で評価する。$V=|\bm{v}^B|$ は対気速度、$d$ は機体直径である。
\begin{eqnarray}
c_L &=& \frac{Q\,S\,C_{N\alpha}\,\ell^2}{V} \\
c_{Cmq} &=& -\frac{Q\,S\,d^2\,C_{mq}}{2V} \\
\zeta &=& \frac{c_L + c_{Cmq}}{2\sqrt{k_\alpha\,I_T}}
\end{eqnarray}
共振点近傍での増幅率は $A_{res} = 1/(2\zeta)$ で見積もる。

ジャイロ安定係数 $S_g$ と、動的安定係数 $S_d$ およびその安定境界を次で評価する。動的安定では Magnus モーメント係数 $C_{mp\alpha}$ と $C_{m\dot\alpha}$ が本来関与するが、本モデルでは未実装のため $0$ として扱う。$m$ は全機質量である。
\begin{eqnarray}
S_g &=& \frac{(I_{xx}\,p)^2}{4\,I_T\,k_\alpha} \\
S_d &=& \frac{2\,C_{N\alpha}}{C_{N\alpha} - C_A - \dfrac{m\,d^2}{I_T}\,C_{mq}}
\end{eqnarray}
動的安定条件は $1/S_g < S_d(2 - S_d)$ である。$S_g > 1$ でジャイロ安定となるが、フィン静安定主体（$I_{xx} \ll I_T$）の機体では $S_g \ll 1$ となるため、$S_g, S_d$ はスピンを積極的に用いる設計での参考指標と位置づける。

スピンアップ方向のフィンカントロールモーメントとロール減衰モーメントの釣り合いから定まる平衡スピンレート $p_{eq}$ は、フィンカント角 $\delta$ を用いて
\begin{equation}
p_{eq} = -\frac{C_{ld}\,\delta\,2V}{C_{lp}\,d}, \qquad f_{s,eq} = \frac{|p_{eq}|}{2\pi}
\end{equation}
となる。$f_{s,eq}$ と $f_n$ の近接は、定常スピンレートが共振点に捕捉（ロックイン）する危険性の指標となる。

構成非対称による強制モーメント $M_{asym}$ は、推力作用点・重心の横方向オフセットによる推力モーメントの横成分と、慣性乗積 $I_{xy}, I_{xz}$（主軸ミスアライメント）によるスピン依存強制 $p^2\sqrt{I_{xy}^2 + I_{xz}^2}$ の和で評価する。
\begin{equation}
M_{asym} = \sqrt{(M^B_{T,y})^2 + (M^B_{T,z})^2} + p^2\sqrt{I_{xy}^2 + I_{xz}^2}
\end{equation}
これを共振応答倍率で増幅したトリム迎角 $\alpha_{trim}$、および横空力荷重の代理量 $N_{lat}$ を
\begin{eqnarray}
\alpha_{trim} &=& \frac{M_{asym}}{k_\alpha\,\sqrt{(1 - \Lambda^2)^2 + (2\zeta\Lambda)^2}} \\
N_{lat} &=& Q\,S\,C_{N\alpha}\,\alpha_t
\end{eqnarray}
で出力する。$\alpha_{trim}$ は線形理論による解析推定値であり、$\Lambda \to 1$ で分母が小さくなり急増する。実機挙動の確認には 6-DOF 積分結果の $\alpha_t$ 包絡線と併せて評価すること。

\section{Ref}
\begin{itemize}
  \item Peter, H., "Modeing and Simulation of Aerospace Vehicle Dynamics"
  \item 戸川 隼人, 石黒 登美子, 山本 浩通, "スピンを伴うロケットの運動を計算するプログラム", 航空宇宙技術研究所資料 TM-145, 1968
  \item 秋葉 鐐二郎, 松尾 弘毅, 佐伯 信吾, "L-3H-1,2,3およびL-4S-1,2の性能計算", 東京大学宇宙航空研究所報告, 1967, vol.3, no.1, p.173-182
  \item 山口 功, 木田 隆, 岡本 修, 狼 嘉彰, "クォータニオンとオイラー角によるキネマティックス表現の比較について", 航空宇宙技術研究所資料 TM-636, 1991
  \item National Imagery and Mapping Agency, "Department of Defense World Geodetic System 1984 Its Definition and Relationships with Local Geodetic System", National Imagery and Mapping Agency Technical Report 8350.2
  \item U.S. Government Pringing Office, "U.S. Standard Atmosphere", 1976
  \item 長坂 直彦, "回転楕円体上の測地線及び航程線の算出について", 海上保安庁海洋情報部研究報告, 2013, 第50号, p.37-57
  \item Ahn, J., Roh, W., "Noniterative Instantaneous Impact Point Prediction Algorithm for Launch Operations", Journal of Guidance, Control, and Dynamics, 2012, vol.35, no.2, p.645-648
  \item Dormand, J. R., Prince, P. J., "A family of embedded Runge-Kutta formulae", Journal of Computational and Applied Mathematics, 6 (1): p.19–26
\end{itemize}

\end{document}